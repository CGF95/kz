...
/* functions */
z2_extern void z2_LoadOverlay(uint32_t vrom_start, uint32_t vrom_end,
                              uint32_t vram_start, uint32_t vram_end,
                              void *dest);
z2_extern int32_t osEPiStartDma(OSPiHandle *pihandle, OSIoMesg *mb,
                                int32_t direction);
z2_extern int32_t osEPiReadIo(OSPiHandle *pihandle, uint32_t devAddr,
                              uint32_t *data);
z2_extern int32_t osEPiWriteIo(OSPiHandle *pihandle, uint32_t devAddr,
                               uint32_t data);
z2_extern void z2_DrawActors(z2_game_t *game, z2_actor_ctxt_t *actor_ctx);
z2_extern z2_actor_t *z2_SpawnActor(z2_actor_ctxt_t *actor_ctx, z2_game_t *game,
                                    uint16_t actor_id, float x, float y,
                                    float z, z2_angle_t rx, z2_angle_t ry,
                                    z2_angle_t rz, int16_t actor_variable,
                                    int camera_cmd_idx, int spawn_time_flags,
                                    void *param_13);
z2_extern void z2_DeleteActor(z2_actor_ctxt_t *actor_ctx, z2_actor_t *actor,
                              z2_game_t *game);
z2_extern void z2_CreateStaticCollision(z2_col_ctxt_t *col_ctx, z2_game_t *game,
                                        z2_col_lut_t *col_lut);
...
z2_extern void z2_ActionLabelUpdate(z2_hud_ctxt_t *hud_ctx, uint16_t action,
                                    int btn_idx);
z2_extern void z2_LoadRoom(z2_game_t *game, z2_room_ctxt_t *room_ctx,
                           uint8_t room_id);
z2_extern void z2_DrawRoom(z2_game_t *game, z2_room_t *room, uint32_t flags);
...
z2_extern void z2_LoadSkyboxData(z2_game_t *game, z2_skybox_ctxt_t *sky_ctx,
                                 int16_t type);
...
#endif
z2_extern void z2_LoadArchiveFile2(uint32_t rom, int arg1, void *ram,
                                   size_t size);
...
/* CutsceneManager scene list ptr/count + waiting/next queues (0x30 bytes) */
z2_extern char z2_cs_scene_lists[0x30];
/* z_demo.c sCurTextId + sCurOcarinaAction */
z2_extern char z2_cs_text_state[8];
/* z_horse.c gHorseIsMounted, D_801BDAA0, gHorsePlayedEponasSong */
z2_extern char z2_horse_state[0xC];
