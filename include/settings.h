...
#define SETTINGS_SIZE                                                          \
  (sizeof(struct settings_header) + sizeof(struct settings_data))
#define SETTINGS_PAD                                                           \
  ((IO_BLOCK_SIZE - (SETTINGS_SIZE & (IO_BLOCK_SIZE - 1))) &                   \
   (IO_BLOCK_SIZE - 1))
#define SETTINGS_MAX 3
#define FULL_SETTINGS                                                          \
  ((uint32_t)(((uint32_t)Z2_VERSION << 16) | ((uint32_t)KZ_VERSION << 8) |     \
