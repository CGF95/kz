...
typedef int (*menu_event_cb_t)(event_handler_t *handler, menu_event_t event,
                               void **event_data);
typedef int (*menu_item_event_cb_t)(event_handler_t *handler,
...
/* event functions */
event_handler_t *menu_register_event(menu_t *menu, menu_event_t event,
                                     menu_event_cb_t callback,
                                     void *callback_data);
...
event_handler_t *menu_item_register_event(menu_item_t *item, menu_event_t event,
                                          menu_item_event_cb_t callback,
                                          void *callback_data);
void menu_item_unregister_event(menu_item_t *item, menu_event_t event);
int menu_item_trigger_event(menu_item_t *item, menu_event_t event,
                            void **event_data);

void menu_init(menu_t *menu, int16_t x_offset, int16_t y_offset);
void menu_draw(menu_t *menu);
int menu_return(event_handler_t *handler, menu_event_t event,
                void **event_data);
...
void menu_item_offset_set(menu_item_t *item, int16_t offset_x,
                          int16_t offset_y);
void menu_input(menu_t *menu, void **event_data);

menu_item_t *menu_label_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                            void *text);
menu_item_t *menu_submenu_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                              char *text, menu_t *submenu);
menu_item_t *menu_checkbox_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y);
menu_item_t *menu_button_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                             char *text, menu_item_event_cb_t on_activate,
                             void *activate_data);
menu_item_t *menu_gfx_button_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                                 menu_sprite_t *sprite,
                                 menu_item_event_cb_t on_activate,
                                 void *activate_data);
menu_item_t *menu_static_gfx_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                                 gfx_texture *texture, int tile, int width,
                                 int height);
menu_item_t *menu_list_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                           char **text, int option_cnt);
menu_item_t *menu_watch_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                            watch_t *watch, _Bool setpos);
menu_item_t *menu_text_input_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
...
menu_item_t *menu_text_input_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                                 char *default_text, char **value_ptr,
                                 int val_len);
menu_item_t *menu_move_button_add(menu_t *menu, uint16_t cell_x,
                                  uint16_t cell_y, int16_t *move_x,
                                  int16_t *move_y);
menu_item_t *menu_number_input_add(menu_t *menu, uint16_t cell_x,
                                   uint16_t cell_y, int8_t base,
                                   uint8_t length);
menu_item_t *menu_switch_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                             ...
                             uint8_t on_tile, uint8_t off_tile, int width,
                             int height, char *tooltip);
menu_item_t *menu_item_list_add(menu_t *menu, uint16_t cell_x, uint16_t cell_y,
                                uint16_t start_tile, int8_t *options,
                                uint8_t option_cnt, int8_t *value_ptr,
                                int8_t *ovl_values, int tiles_cnt,
...
int menu_number_word_event(event_handler_t *handler, menu_event_t event,
                           void **event_data);
int menu_number_halfword_event(event_handler_t *handler, menu_event_t event,
                               void **event_data);
int menu_number_byte_event(event_handler_t *handler, menu_event_t event,
                           void **event_data);
