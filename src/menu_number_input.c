...
static uint32_t get_val(struct item_data *data) { return data->value; }
...
static int menu_number_event(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
...
menu_item_t *menu_number_input_add(menu_t *menu, uint16_t x, uint16_t y,
  ...
  if (item) {
    ...
    menu_item_register_event(
        item, MENU_EVENT_ACTIVATE | MENU_EVENT_NAVIGATE | MENU_EVENT_UPDATE,
        menu_number_event, NULL);
  }
  return item;
}