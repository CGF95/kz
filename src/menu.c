...
static void menu_navigate(menu_t *menu, menu_nav_t nav) {
  ...
  if (menu->selected_item != NULL &&
      menu_item_trigger_event(menu->selected_item, MENU_EVENT_NAVIGATE,
                              (void *)nav)) {
    return;
  }
  ...
  for (menu_item_t *item = menu->items.first; item != NULL;
       item = list_next(item)) {
    ...
    int pa = dir_horiz ? dir_horiz * distance_x
                       : (dir_vert ? dir_vert * distance_y : 0);
    ...
    if (pa > 0 &&
        (near == NULL || pe < ndist_pe || (pe == ndist_pe && pa < ndist_pa))) {
      ndist_pa = pa;
      ndist_pe = pe;
      near = item;
    }
    ...
    if (pa < 0 &&
        (far == NULL || -pa > fdist_pa || (-pa == fdist_pa && pe < fdist_pe))) {
      far = item;
      fdist_pa = -pa;
      fdist_pe = pe;
    }
...
int menu_item_x(menu_item_t *item) {
  return menu_x(item->owner) + (item->x_cell * item->owner->cell_width) +
         (item->x_cell * item->owner->x_padding) + item->x_offset;
}

int menu_item_y(menu_item_t *item) {
  return menu_y(item->owner) + (item->y_cell * item->owner->cell_height) +
         (item->y_cell * item->owner->y_padding) + item->y_offset;
}
...
void menu_draw(menu_t *menu) {
  ...
  for (menu_item_t *item = menu->items.first; item != NULL;
    ...
    gfx_printf_color(menu_item_x(item), menu_item_y(item), color, "%s",
                     item->text);
  }
}
...
int menu_item_trigger_event(menu_item_t *item, menu_event_t event,
                            void **event_data) {
  // run all handlers for this event, and return if one of them considers it
  // handled
  for (event_handler_t *handler = item->owner->item_events.first;
       handler != NULL; handler = list_next(handler)) {
    if (handler->subscriber == item && (handler->event & event) == event &&
...
int menu_trigger_event(menu_t *menu, menu_event_t event, void **event_data) {
  ...
  if (menu->child != NULL &&
      menu_trigger_event(menu->child, event, event_data)) {
    return true;
  }

  // run all handlers for this event, and return if the event is considered
  // handled
  for (event_handler_t *handler = menu->events.first; handler;
       handler = list_next(handler)) {
    if (handler->subscriber == menu && (handler->event & event) == event &&
  ...
  case MENU_EVENT_ACTIVATE:
    menu_item_trigger_event(menu->selected_item, MENU_EVENT_ACTIVATE,
                            event_data);
    break;
  case MENU_EVENT_ENTER: {
    if (event_data != NULL) {
      ...
      kz.tooltip = NULL;
      menu_item_trigger_event(submenu->selected_item, MENU_EVENT_ENTER,
                              event_data);
      menu->child = submenu;
      submenu->parent = menu;
    }
  ...
  case MENU_EVENT_NAVIGATE:
    if (menu_item_trigger_event(menu->selected_item, MENU_EVENT_NAVIGATE,
                                event_data)) {
      return true;
    }
  ...
  case MENU_EVENT_RETURN: {
    if (menu->parent != NULL) {
      ...
      menu_item_trigger_event(menu->parent->selected_item, MENU_EVENT_ENTER,
                              event_data);
...
void menu_item_offset_set(menu_item_t *item, int16_t x_offset,
                          int16_t y_offset) {
...
menu_item_t *menu_label_add(menu_t *menu, uint16_t x_cell, uint16_t y_cell,
                            void *text) {
