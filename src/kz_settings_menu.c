...
static int profile_dec_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
...
static int profile_inc_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
  kz.settings_profile++;
  kz.settings_profile %= SETTINGS_MAX;
  return 1;
...
static int memfile_dec_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
  command_prev_memfile();
  return 1;
}

static int memfile_inc_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
  command_next_memfile();
  return 1;
}

static int position_dec_onactivate(event_handler_t *handler, menu_event_t event,
                                   void **event_data) {
  command_prev_position();
  return 1;
}

static int position_inc_onactivate(event_handler_t *handler, menu_event_t event,
                                   void **event_data) {
  command_next_position();
  return 1;
}

static int save_profile_onactivate(event_handler_t *handler, menu_event_t event,
                                   void **event_data) {
  save_settings_to_flashram(kz.settings_profile);
  return 1;
}

static int load_profile_onactivate(event_handler_t *handler, menu_event_t event,
                                   void **event_data) {
...
static int default_settings_onactivate(event_handler_t *handler,
...
static int command_onactivate(event_handler_t *handler, menu_event_t event,
                              void **event_data) {
...
static int pause_help_event(event_handler_t *handler, menu_event_t event,
                            void **event_data) {
...
static int settings_switch_event(event_handler_t *handler, menu_event_t event,
                                 void **event_data) {
  enum settings_switch_item sw =
      (enum settings_switch_item)handler->callback_data;
  if (event == MENU_EVENT_ACTIVATE) {
    switch (sw) {
...
static void update_commands_table(void) {
  for (int i = 0; i < 13; i++) {
    command_bindings[i].command_button->text =
        kz_commands[i + command_pos].text;
    menu_bind_set(command_bindings[i].command_bind, i + command_pos);
  }
}

static int command_dec_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
  if (command_pos == 0) {
...
static int command_inc_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
...
static int export_pos_onactivate(event_handler_t *handler, menu_event_t event,
                                 void **event_data) {
...
static int import_pos_onactivate(event_handler_t *handler, menu_event_t event,
                                 void **event_data) {
...
static int export_memfile_onactivate(event_handler_t *handler,
                                     menu_event_t event, void **event_data) {
...
static int import_memfile_onactivate(event_handler_t *handler,
                                     menu_event_t event, void **event_data) {
...
static int memfile_action_onlist(event_handler_t *handler, menu_event_t event,
                                 void **event_data) {
...
menu_t *create_settings_menu(void) {
  ...
  settingsm.selected_item =
      menu_button_add(&settingsm, 0, 0, "return", menu_return, NULL);
  ...
  menu_button_add(&settingsm, 0, 2, "save settings", save_profile_onactivate,
                  NULL);
  menu_button_add(&settingsm, 0, 3, "load settings", load_profile_onactivate,
                  NULL);
  menu_button_add(&settingsm, 0, 4, "load default settings",
                  default_settings_onactivate, NULL);
  menu_item_t *item = menu_checkbox_add(&settingsm, 0, 5);
  menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                           pause_help_event, NULL);
  menu_label_add(&settingsm, 2, 5, "display pause help text");

  item = menu_checkbox_add(&settingsm, 0, 6);
  menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                           settings_switch_event, (void *)SW_INPUT_DISPLAY);
  menu_checkbox_set(item, settings->input_display);
  ...
  menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                           settings_switch_event, (void *)SW_TIMER);
  menu_checkbox_set(item, settings->timer);
  menu_label_add(&settingsm, 2, 7, "timer");
  menu_move_button_add(&settingsm, 16, 7, &settings->timer_x,
                       &settings->timer_y);

  item = menu_checkbox_add(&settingsm, 0, 8);
  menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                           settings_switch_event, (void *)SW_LAG_COUNTER);
  menu_checkbox_set(item, settings->lag_counter);
  ...
  menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                           settings_switch_event, (void *)SW_TURBO);
  menu_checkbox_set(item, settings->turbo_type);
  ...
  int y = 2;
  {
    ...
    commands.selected_item =
        menu_button_add(&commands, 0, 0, "return", menu_return, NULL);
    menu_label_add(&commands, 2, 1, "command");
    menu_label_add(&commands, 22, 1, "bind");

    menu_gfx_button_add(&commands, 0, 2, scroll_up_sprite,
                        command_dec_onactivate, NULL);
    menu_gfx_button_add(&commands, 0, 14, scroll_down_sprite,
                        command_inc_onactivate, NULL);

    for (int i = 0; i < 13; i++) {
      command_bindings[i].command_button = menu_button_add(
          &commands, 2, y, kz_commands[i].text, command_onactivate, (void *)i);
  ...
  settings_page2.selected_item =
      menu_button_add(&settings_page2, 0, 0, "prev page", menu_return, NULL);
  ...
  y = 2;
#ifndef LITE
  menu_gfx_button_add(&settings_page2, 0, y, pexport_sprite,
                      export_memfile_onactivate, NULL)
      ->tooltip = "export memfile";
  menu_gfx_button_add(&settings_page2, 4, y, pimport_sprite,
                      import_memfile_onactivate, NULL)
      ->tooltip = "import memfile";
  y += 2;
#endif

  menu_label_add(&settings_page2, 0, y, "memfile action");
  item = menu_list_add(&settings_page2, 15, y++, memfile_action_text,
                       sizeof(memfile_action_values) /
                           sizeof(*memfile_action_values));
...
#ifndef LITE
  menu_gfx_button_add(&settings_page2, 0, y, pexport_sprite,
                      export_pos_onactivate, NULL)
      ->tooltip = "export position";
  menu_gfx_button_add(&settings_page2, 4, y, pimport_sprite,
                      import_pos_onactivate, NULL)
      ->tooltip = "import position";
#endif
