...
static struct equip_option sword_options[] = {{0, 1, Z2_ITEM_KOKIRI_SWORD},
                                              {0, 2, Z2_ITEM_RAZOR_SWORD},
                                              {0, 3, Z2_ITEM_GILDED_SWORD}};

static struct equip_option shield_options[] = {{4, 1, Z2_ITEM_HERO_SHIELD},
...
static int b_button_event(event_handler_t *handler, menu_event_t event,
                          void **event_data) {
...
static int c_button_onactivate(event_handler_t *handler, menu_event_t event,
                               void **event_data) {
...
static void equip_option_draw(menu_item_t *item) {
  ...
  gfx_draw_sprite(get_item_texture(data->item_id, 0), x, y,
                  get_equip_value(data) == data->val ? 0 : 1, 16, 16);
}

static int equip_option_onactivate(event_handler_t *handler, menu_event_t event,
                                   void **event_data) {
...
static menu_item_t *equip_option_add(menu_t *menu, uint16_t x_cell,
                                     uint16_t y_cell,
                                     struct equip_option *option) {
  menu_item_t *item = menu_add(menu, x_cell, y_cell);
  if (item) {
    item->interactive = 1;
    item->draw_proc = equip_option_draw;
    item->data = option;
    menu_item_register_event(item, MENU_EVENT_ACTIVATE, equip_option_onactivate,
                             option);
  }
  return item;
}
...
menu_t *create_equips_menu(void) {
  ...
  equips.selected_item =
      menu_button_add(&equips, 0, 0, "return", menu_return, NULL);
  ...
  static menu_sprite_t b_sprite = {NULL,       0,  0,  COLOR_GREEN,
                                   0x64FF78FF, 22, 22, NULL,
                                   0x64FF78FF, 0,  0,  NULL};

  static menu_sprite_t c_left_sprite = {NULL,       0,  0,  COLOR_GREEN,
                                        0xFFF000FF, 16, 16, NULL,
                                        0xFFF000FF, 0,  2,  NULL};

  static menu_sprite_t c_down_sprite = {NULL,       0,  0,  COLOR_GREEN,
                                        0xFFF000FF, 16, 16, NULL,
                                        0xFFF000FF, 0,  3,  NULL};

  static menu_sprite_t c_right_sprite = {NULL,       0,  0,  COLOR_GREEN,
                                         0xFFF000FF, 16, 16, NULL,
                                         0xFFF000FF, 0,  4,  NULL};
  ...
  item = menu_item_list_add(&equips, 0, 3, 0, NULL, Z2_ITEM_ODOLWAS_REMAINS - 1,
                            &b_val, NULL, Z2_ITEM_ODOLWAS_REMAINS - 1,
                            &b_sprite, NULL);
  menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                           b_button_event, NULL);
  menu_item_offset_set(item, 0, 8);

  item = menu_item_list_add(&equips, 1, 3, 0, NULL, Z2_ITEM_ODOLWAS_REMAINS - 1,
                            &z2_file.form_button_item[0].cleft, NULL,
                            Z2_ITEM_ODOLWAS_REMAINS - 1, &c_left_sprite, NULL);
  menu_item_register_event(item, MENU_EVENT_ACTIVATE, c_button_onactivate,
                           (void *)1);
  menu_item_offset_set(item, 6, 11);

  item = menu_item_list_add(&equips, 2, 3, 0, NULL, Z2_ITEM_ODOLWAS_REMAINS - 1,
                            &z2_file.form_button_item[0].cdown, NULL,
                            Z2_ITEM_ODOLWAS_REMAINS - 1, &c_down_sprite, NULL);
  menu_item_register_event(item, MENU_EVENT_ACTIVATE, c_button_onactivate,
                           (void *)2);
  menu_item_offset_set(item, 4, 21);

  item = menu_item_list_add(&equips, 3, 3, 0, NULL, Z2_ITEM_ODOLWAS_REMAINS - 1,
                            &z2_file.form_button_item[0].cright, NULL,
                            Z2_ITEM_ODOLWAS_REMAINS - 1, &c_right_sprite, NULL);
  menu_item_register_event(item, MENU_EVENT_ACTIVATE, c_button_onactivate,
                           (void *)3);
