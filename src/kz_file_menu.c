...
static int time_of_day_event(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
  uint16_t value = (uint16_t)((uint32_t)*event_data);
  if (event == MENU_EVENT_NUMBER) {
    z2_file.time_of_day = value;
    for (z2_actor_t *actor = z2_game.actor_ctxt.actor_list[0].first; actor;
         actor = actor->next) {
      if (actor->id == 0x15A) {
        if (z2_file.daynight == 1 && (value >= 0xC000 || value < 0x4000)) {
          z2_file.daynight = 0;
        } else if (z2_file.daynight == 0 &&
                   (value <= 0xC000 || value >= 0x4000)) {
          z2_file.daynight = 1;
        }
...
static int intro_watched_event(event_handler_t *handler, menu_event_t event,
                               void **event_data) {
...
static int great_spin_event(event_handler_t *handler, menu_event_t event,
                            void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
    z2_file.week_event_inf[0x17] ^= 0x2;
  } else if (event == MENU_EVENT_UPDATE) {
    menu_checkbox_set(handler->subscriber,
                      (z2_file.week_event_inf[0x17] & 0x2) == 0x2);
  }
  return 0;
}

static int have_tatl_event(event_handler_t *handler, menu_event_t event,
                           void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
...
static int stored_song_event(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
  if (event == MENU_EVENT_UPDATE) {
    int idx = 0;
    for (int i = 0;
         i < sizeof(stored_song_values) / sizeof(*stored_song_values); i++) {
