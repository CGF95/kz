...
static _Bool update_file_list(void) {
  ...
  while ((dirent = readdir(dir)) != NULL) {
    ...
    if (!isdir &&
        (name_len < file_menu_extension_len ||
         !stricmp(&dirent->dir_name[name_len - file_menu_extension_len],
                  file_menu_extension))) {
      continue;
    }
...
static int reset_disk_onactivate(event_handler_t *handler, menu_event_t event,
                                 void **event_data) {
  reset_disk();
  update_view();
  file_menu_offset = 0;
...
static int clear_onactivate(event_handler_t *handler, menu_event_t event,
                            void **event_data) {
  memset(file_menu_text_value, 0, 32);
  return 1;
}

static int menu_file_onactivate(event_handler_t *handler, menu_event_t event,
                                void **event_data) {
...
static int new_folder_event(event_handler_t *handler, menu_event_t event,
                            void **event_data) {
...
static int file_menu_up_onactivate(event_handler_t *handler, menu_event_t event,
                                   void **event_data) {
...
static int file_menu_down_onactivate(event_handler_t *handler,
...
static void menu_file_init(void) {
  ...
  file_menu.selected_item =
      menu_button_add(&file_menu, 0, 0, "return", menu_return, NULL);
  ...
  file_menu_text_entry = menu_text_input_add(&file_menu, 0, 3, "untitled",
                                             &file_menu_text_value, 32);
  file_menu_accept_button =
      menu_button_add(&file_menu, 0, 4, "accept", accept_onactivate, NULL);
  file_menu_clear_button =
      menu_button_add(&file_menu, 8, 4, "clear", clear_onactivate, NULL);
  file_menu_new_folder = menu_add(&file_menu, 0, 5);
  file_menu_new_folder->draw_proc = menu_new_folder_draw;
  menu_item_register_event(file_menu_new_folder,
                           MENU_EVENT_ACTIVATE | MENU_EVENT_KEYBOARD,
                           new_folder_event, NULL);
  file_menu_new_folder->interactive = 1;
  int y = 3;

  file_menu_up_button = menu_gfx_button_add(&file_menu, 0, y, scroll_up_sprite,
                                            file_menu_up_onactivate, NULL);
  file_menu_down_button =
      menu_gfx_button_add(&file_menu, 0, y + FILE_ROWS - 1, scroll_down_sprite,
                          file_menu_down_onactivate, NULL);
  ...
  for (int i = 0; i < FILE_ROWS; i++) {
    ...
    item->interactive = 1;
    menu_item_register_event(item, MENU_EVENT_ACTIVATE, menu_file_onactivate,
                             NULL);
...
void menu_file_get(file_mode_t mode, const char *default_fn,
                   const char *extension, get_file_callback_t callback,
                   void *callback_data) {
  if (!file_menu_ready) {
    menu_file_init();
  }
