...
static int warp_onactivate(event_handler_t *handler, menu_event_t event,
                           void **event_data) {
...
menu_t *create_warps_menu(void) {
  ...
  warps.selected_item =
      menu_button_add(&warps, 0, 0, "return", menu_return, NULL);
  for (int i = 0;
       i < sizeof(scene_categories) / sizeof(struct kz_scene_category); i++) {
    ...
    cat_menu->selected_item =
        menu_button_add(cat_menu, 0, 0, "return", menu_return, NULL);
    ...
    for (int j = 0; j < cat.scene_cnt; j++) {
      ...
      if (scene.entrance_cnt > 1) {
        ...
        entrance_menu->selected_item =
            menu_button_add(entrance_menu, 0, 0, "return", menu_return, NULL);
        for (int k = 0; k < scene.entrance_cnt; k++) {
          menu_button_add(
              entrance_menu, 0, k + 1, scene.entrances[k], warp_onactivate,
              (void *)(((scene.scene & 0xFF) << 8) | ((k & 0xFF) << 4)));
        }
        menu_submenu_add(cat_menu, 0, j + 1, scene.scene_name, entrance_menu);
      } else {
        menu_button_add(
            cat_menu, 0, j + 1, scene.scene_name, warp_onactivate,
            (void *)(((scene.scene & 0xFF) << 8) | ((0 & 0xFF) << 4)));
      }
    }
  }
  return &warps;
