...
_Bool load_overlay(void **src, void **tab_addr, uint32_t vrom_start,
                   uint32_t vrom_end, uint32_t vram_start, uint32_t vram_end) {
  FORCE_CLEAN(load_overlay);
  z2_file_table_t *file = NULL;
  for (int i = 0;; i++) {
    if (z2_file_table[i].vrom_start == vrom_start &&
        z2_file_table[i].vrom_end == vrom_end) {
  ...
  uint32_t *overlay_header_offset =
      (void *)(end - sizeof(*overlay_header_offset));
  ...
  size_t rom_sz = file->vrom_end - file->vrom_start;
  void *rom_buf = malloc(rom_sz);
  if (!rom_buf) return 0;
  zu_file_load(vrom_start, rom_buf, rom_sz);
  char *rom_data = (char *)rom_buf + overlay_hdr->text_size;

  /* reconstruct data section from RLE delta */
  for (uint32_t i = 0; i < overlay_hdr->data_size;) {
    uint16_t n_copy = 0, n_save = 0;
    st_read(src, &n_copy, sizeof(n_copy));
    st_read(src, &n_save, sizeof(n_save));
    memcpy(&data[i], &rom_data[i], n_copy);
    i += n_copy;
    st_read(src, &data[i], n_save);
    i += n_save;
  }
...
void load_state(void *state) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (hdr.state_version >= 10) {
      st_read(&p, &info->unk_1C8, sizeof(info->unk_1C8));
      st_read(&p, &info->chan_data, sizeof(info->chan_data));
      st_read(&p, &info->tail, sizeof(info->tail));
    }
  }
  ...
  if (cur_config != saved_cfg) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (info->seq_idx == 0xFFFF || !info->active ||
          info->seq_idx != ctl->seq_idx) {
  ...
  for (int i = 0; i < ovl_cnt; i++) {
    ...
    if (i == next_ent) {
      ...
      load_overlay(&p, &ent->ram, ent->vrom_start, ent->vrom_end,
                   ent->vram_start, ent->vram_end);
      st_read(&p, &next_ent, sizeof(next_ent));
  ...
  for (int i = 0;
       i < sizeof(z2_extern_scene_table) / sizeof(*z2_extern_scene_table);
       i++) {
    z2_extern_ent_tab_t *extern_ent = &z2_extern_scene_table[i];
    st_read(&p, extern_ent->entrances,
            sizeof(*extern_ent->entrances) * extern_ent->ent_cnt);
...

  {
    ...
    st_read(&p, *z2_minimap_ctx.room_info,
            sizeof(**z2_minimap_ctx.room_info) * z2_game.room_cnt);

    z2_minimap_room_info_t *room_info =
        &(*z2_minimap_ctx.room_info)[z2_minimap_ctx.room];

    if (room_info->map != 0xFFFF) {
      uint32_t tile_size = z2_get_mmap_tile_size(room_info->map);
      if (tile_size != 0) {
        z2_LoadArchiveFile2(z2_file_table[z2_minimap_assets].vrom_start,
                            room_info->map - 0x100, z2_minimap_ctx.cur_tex,
                            tile_size);
      }
    }
    ...
    if (z2_minimap_ctx.trnsn_room != -1) {
      ...
      if (room_info->map != 0xFFFF) {
        ...
        if (tile_size != 0) {
          z2_LoadArchiveFile2(z2_file_table[z2_minimap_assets].vrom_start,
                              room_info->map - 0x100, z2_minimap_ctx.prev_tex,
                              tile_size);
        }
      }
    }
  ...
  /* load scene file, and create static collision if loading from a different
   * scene */
  ...
  if (scene_index != z2_game.scene_index) {
    ...
    while (cmd->cmd != 0x14) {
      ...
      if (cmd->cmd == 0x11) {
        ...
        zu_file_load(file->vrom_start, z2_game.room_ctx.area_map_tex,
                     file->vrom_end - file->vrom_start);
      }
      cmd++;
    }
    z2_game.col_ctxt.stc_list_pos = 0;
    z2_CreateStaticCollision(&z2_game.col_ctxt, &z2_game,
                             z2_game.col_ctxt.stc_lut);
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (c_ptr && c_id != -1 &&
        (p_id != c_id || z2_game.scene_index != scene_index ||
         p_ptr != c_ptr)) {
  ...
  zu_file_load(z2_file_table[z2_do_action_static].vrom_start + 0x480,
               z2_game.hud_ctx.do_action_static + 0x300, 0x180);

  if (z2_game.pause_ctx.state != 0) {
    /* we need to load pause screen assets */
#if Z2_VERSION == NZSE
    z2_LoadArchiveFile(z2_file_table[z2_item_icon_archive].vrom_start,
                       z2_game.pause_ctx.icon_item_static, 0);
    z2_LoadArchiveFile(z2_file_table[z2_icon_item_24_static].vrom_start,
                       z2_game.pause_ctx.icon_item_24, 0);
    uint32_t rom =
        z2_game.pause_ctx.screen_idx == 1 ? z2_location_names : z2_item_names;
    z2_LoadArchiveFile2(z2_file_table[rom].vrom_start,
                        z2_game.pause_ctx.selected_item,
                        z2_game.pause_ctx.icon_name_text, 0x400);
    ...
    z2_DecodeArchiveFile(z2_file_table[z2_item_names].prom_start,
                         z2_game.pause_ctx.selected_item,
                         z2_game.pause_ctx.icon_name_text, 0x400);
#endif

    zu_file_load(z2_file_table[z2_do_action_static].vrom_start + 0x480,
                 z2_game.hud_ctx.do_action_static + 0x480, 0x180);
    zu_file_load(z2_file_table[z2_do_action_static].vrom_start + 0x480,
                 z2_game.hud_ctx.do_action_static + 0x300, 0x180);
    if (z2_get_map_type(&z2_game) == 0) {
      zu_file_idx_load(z2_icon_item_field_static,
                       z2_game.pause_ctx.icon_item_map);
    } else {
    ...
    for (int i = 0; i < Z2_END_MASK; i++) {
      if (!z2_restriction_table[rest_offset + i]) {
        gfx_texture_desaturate(zu_segment_find(z2_icon_item_static_table[i]),
                               0x1000);
      }
    }
    ...
    for (int i = 0;
         i < sizeof(z2_game.col_ctxt.actors) / sizeof(*z2_game.col_ctxt.actors);
         i++) {
  ...
  if (z2_file
          .form_button_item[z2_file.current_form == 4 ? 0
                                                      : z2_file.current_form]
          .b != Z2_ITEM_NULL) {
  ...
  uint32_t vrom = z2_file_table[z2_clock_face_days].vrom_start +
                  (z2_file.day == 0 ? 0 : z2_file.day - 1) * 0x510;
  void *ram = (char *)z2_game.hud_ctx.do_action_static + 0x780;
  zu_file_load(vrom, ram, 0x510);

  /* load message box textures */
  zu_file_load(z2_file_table[z2_message_static].vrom_start +
                   z2_message_static_offsets[z2_game.message_bg_type] * 0x1000,
               z2_game.message_bg_tex, 0x1000);
  ...
  z2_disp_buf_t *z2_dlist[5] = {
      &gfx->work,    &gfx->poly_opa,      &gfx->poly_xlu,
      &gfx->overlay, &gfx->unk_0x1B8_buf,
  };
  for (int i = 0; i < 5; i++) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (i == 4 && ctl->seq_idx == 0x0001) {
      ...
      for (int j = 0; j < 16; j++) {
        if (!(night_sfx->unk_0x02 & (1 << j)) &&
            (night_sfx->unk_0x00 & (1 << j))) {
    ...
    } else if (info->seq_idx != 0xFFFF && info->active &&
               !(sequencer->enabled)) {
    ...
    for (int j = 0; j < 16; j++) {
      ...
      zu_seq_chan_vol(i, j, 1.0f);
      zu_seq_chan_mute(i, j, is_mute);
    }

    if (hdr.state_version >= 10) {
      memcpy(&ctl->unk_0x1C8, &info->unk_1C8, sizeof(ctl->unk_0x1C8));
      memcpy(ctl->unk_0x00, info->chan_data, sizeof(ctl->unk_0x00));
      memcpy(&ctl->timer, info->tail, sizeof(info->tail));
    }
  }
  ...
  /* state version 9 additions */
  if (hdr.state_version >= 9) {
    st_read(&p, z2_cs_scene_lists, sizeof(z2_cs_scene_lists));
    st_read(&p, z2_cs_text_state, sizeof(z2_cs_text_state));
    st_read(&p, z2_horse_state, sizeof(z2_horse_state));
  }
}

_Bool save_overlay(void **dst, void *src, uint32_t vrom_start,
                   uint32_t vrom_end) {
  ...
  /* RLE delta-encode data section against ROM */
  uint16_t n_copy = 0, n_save = 0;
  char *save_start = NULL;
  for (uint32_t i = 0; i < overlay_hdr->data_size; i++) {
    if (rom_data[i] == data[i]) {
      if (n_save > 0) {
        st_write(dst, &n_copy, sizeof(n_copy));
        st_write(dst, &n_save, sizeof(n_save));
        st_write(dst, save_start, n_save);
        n_copy = 0; n_save = 0;
      }
      n_copy++;
    } else {
      if (n_save == 0) save_start = &data[i];
      n_save++;
    }
  }
...
size_t save_state(void *state) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    st_write(&p, &seq_ctl->unk_0x1C8, sizeof(seq_ctl->unk_0x1C8));
    st_write(&p, seq_ctl->unk_0x00, sizeof(seq_ctl->unk_0x00));
    st_write(&p, &seq_ctl->timer, 0x1C);
  }
  ...
  for (int i = 0;
       ...
       i++) {
    z2_extern_ent_tab_t *extern_ent = &z2_extern_scene_table[i];
    st_write(&p, extern_ent->entrances,
             sizeof(*extern_ent->entrances) * extern_ent->ent_cnt);
    for (int j = 0; j < extern_ent->ent_cnt; j++) {
      z2_entrance_rec_t *ent_rec = extern_ent->entrances[j];
  ...
  st_write(&p, *z2_minimap_ctx.room_info,
           sizeof(**z2_minimap_ctx.room_info) * z2_game.room_cnt);
  ...
  st_write(&p, dynamic_col->dyn.list,
           sizeof(*dynamic_col->dyn.list) * dynamic_col->dyn_list_max);
  st_write(&p, dynamic_col->dyn_poly,
           sizeof(*dynamic_col->dyn_poly) * dynamic_col->dyn_poly_max);
  st_write(&p, dynamic_col->dyn_vtx,
           sizeof(*dynamic_col->dyn_vtx) * dynamic_col->dyn_vtx_max);
  ...
  /* state version 9 additions */
  st_write(&p, z2_cs_scene_lists, sizeof(z2_cs_scene_lists));
  st_write(&p, z2_cs_text_state, sizeof(z2_cs_text_state));
  st_write(&p, z2_horse_state, sizeof(z2_horse_state));
...
static struct state_compat compatabilities[] = {
    {.state_version = 8, .compat_versions = {7, -1}},
    {.state_version = 9, .compat_versions = {8, 7, -1}},
    {.state_version = 10, .compat_versions = {9, -1}}};
...
int state_is_valid(kz_state_hdr_t *state) {
  ...
  for (int i = 0; i < sizeof(compatabilities) / sizeof(*compatabilities); i++) {
    ...
    for (int j = 0;
         j < sizeof(compat->compat_versions) / sizeof(*compat->compat_versions);
         j++) {
