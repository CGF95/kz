...
static struct capacity_upgrade_option capacity_options[] = {
    {3, Z2_ITEM_BOMB_BAG_20, 0b111, {0, 2, 3, 4, 0, 0, 0, 0}, -1, "bomb bag"},
    {0, Z2_ITEM_QUIVER_30, 0b111, {0, 3, 4, 5, 6, 2, 3, 4}, -1, "quiver"},
    {12,
...
static struct switch_data owl_data_table[] = {
    {0x0200, "stone tower"},      {0x0100, "ikana canyon"},
    {0x0080, "southern swamp"},   {0x0040, "woodfall"},
    {0x0020, "milk road"},        {0x0010, "clock town"},
    {0x0008, "mountain village"}, {0x0004, "snowhead"},
    {0x0002, "zora cape"},        {0x0001, "great bay"},
    {0x8000, "hidden owl"},
};
...
static int8_t bottle_contents[] = {
    Z2_ITEM_NULL,          Z2_ITEM_BOTTLE,
    Z2_ITEM_RED_POTION,    Z2_ITEM_GREEN_POTION,
    Z2_ITEM_BLUE_POTION,   Z2_ITEM_FAIRY,
    Z2_ITEM_DEKU_PRINCESS, Z2_ITEM_MILK,
    Z2_ITEM_HALF_MILK,     Z2_ITEM_FISH,
    Z2_ITEM_BUGS,          Z2_ITEM_BLUE_FIRE,
    Z2_ITEM_POE,           Z2_ITEM_BIG_POE,
    Z2_ITEM_SPRING_WATER,  Z2_ITEM_HOT_SPRING_WATER,
    Z2_ITEM_ZORA_EGG,      Z2_ITEM_GOLD_DUST,
    Z2_ITEM_MUSHROOM,
#if Z2_VERSION != NZSE
    Z2_ITEM_BOTTLE2,       Z2_ITEM_BOTTLE3,
#endif
    Z2_ITEM_SEAHORSE,      Z2_ITEM_CHATEAU_ROMANI,
    Z2_ITEM_HYLIAN_LOACH,
#if Z2_VERSION == NZSE
    Z2_ITEM_BOTTLE2,
#else
    Z2_ITEM_BOTTLE4,       Z2_ITEM_BOTTLE5,
    Z2_ITEM_BOTTLE6,       Z2_ITEM_BOTTLE7,
    Z2_ITEM_BOTTLE8,       Z2_ITEM_BOTTLE9,
    Z2_ITEM_BOTTLE10,
#endif
};

static int8_t trade_quest_contents[] = {
    Z2_ITEM_NULL,          Z2_ITEM_MOONS_TEAR,
    Z2_ITEM_LAND_DEED,     Z2_ITEM_SWAP_DEED,
    Z2_ITEM_MOUNTAIN_DEED, Z2_ITEM_OCEAN_DEED,
#if Z2_VERSION != NZSE
    Z2_ITEM_POACHERS_SAW,  Z2_ITEM_BROKEN_GORON_SWORD,
    Z2_ITEM_PRESCRIPTION,  Z2_ITEM_SPEEDFROG,
    Z2_ITEM_EYEDROPS,
#endif
    Z2_ITEM_ROOM_KEY,      Z2_ITEM_MAMA_LETTER,
#if Z2_VERSION != NZSE
    Z2_ITEM_MOONS_TEAR2,   Z2_ITEM_MOONS_TEAR3,
    Z2_ITEM_MOONS_TEAR4,   Z2_ITEM_MOONS_TEAR5,
    Z2_ITEM_MOONS_TEAR6,   Z2_ITEM_MOONS_TEAR7,
    Z2_ITEM_MOONS_TEAR8,   Z2_ITEM_MOONS_TEAR9,
#endif
    Z2_ITEM_KAFEI_LETTER,  Z2_ITEM_PENDANT,
#if Z2_VERSION != NZSE
    Z2_ITEM_MOONS_TEAR10,  Z2_ITEM_MOONS_TEAR11,
    Z2_ITEM_MOONS_TEAR12,  Z2_ITEM_MOONS_TEAR13,
    Z2_ITEM_MOONS_TEAR14,  Z2_ITEM_MOONS_TEAR15,
...
static int max_health_event(event_handler_t *handler, menu_event_t event,
                            void **event_data) {
...
static int double_defense_event(event_handler_t *handler, menu_event_t event,
                                void **event_data) {
...
static int magic_event(event_handler_t *handler, menu_event_t event,
                       void **event_data) {
...
static int selected_dungeon_onlist(event_handler_t *handler, menu_event_t event,
                                   void **event_data) {
...
static int dungeon_keys_event(event_handler_t *handler, menu_event_t event,
                              void **event_data) {
  if (event == MENU_EVENT_NUMBER) {
...
static int stray_fairies_event(event_handler_t *handler, menu_event_t event,
                               void **event_data) {
  if (event == MENU_EVENT_NUMBER) {
...
static int dungeon_item_event(event_handler_t *handler, menu_event_t event,
                              void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
    ...
    menu_switch_set(handler->subscriber,
                    (dungeon_items & (uint8_t)((int)handler->callback_data)) >
                        0);
  }
...
static int quest_status_event(event_handler_t *handler, menu_event_t event,
                              void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
    z2_file.quest_status = z2_file.quest_status ^ (int)handler->callback_data;
  } else if (event == MENU_EVENT_UPDATE) {
    menu_switch_set(handler->subscriber,
                    (z2_file.quest_status & (int)handler->callback_data) > 0);
  }

  return 1;
}

static int owl_event(event_handler_t *handler, menu_event_t event,
                     void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
    z2_file.owls_hit = z2_file.owls_hit ^ (int)handler->callback_data;
  } else if (event == MENU_EVENT_UPDATE) {
    menu_switch_set(handler->subscriber,
                    (z2_file.owls_hit & (int)handler->callback_data) > 0);
  }

  return 1;
}

static int capacity_event(event_handler_t *handler, menu_event_t event,
                          void **event_data) {
  struct capacity_upgrade_data *cdata = handler->callback_data;
  struct capacity_upgrade_option *row = &capacity_options[cdata->idx];
  if (event == MENU_EVENT_ACTIVATE) {
    int v = ++cdata->value;
    z2_file.equipment_upgrades =
        (z2_file.equipment_upgrades & ~(row->option_cnt << row->shift)) |
        (v << row->shift);
  } else if (event == MENU_EVENT_UPDATE) {
...
static int item_switch_event(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
...
menu_t *create_inventory_menu(void) {
  ...
  menu.selected_item =
      menu_button_add(&menu, 0, 0, "return", menu_return, NULL);
  ...
  {
    items.selected_item =
        menu_button_add(&items, 0, 0, "return", menu_return, NULL);
    ...
    for (int i = 0; i < item_cnt; i++) {
      ...
      data->map_idx = i;
#ifndef LITE
      item = menu_switch_add(&items, i % 6 + 1, i / 6 + 1,
                             get_item_texture(item_map_table[i].item, 0), NULL,
      ...
      menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                               item_switch_event, data);
    }

    static menu_sprite_t list_sprite = {
        NULL,          0, 0, COLOR_GREEN, DEFAULT_COLOR, 16, 16, NULL,
        DEFAULT_COLOR, 1, 0, NULL,
    };

    list_sprite.background = resource_get(R_Z2_BUTTONS);

    for (int i = 0; i < 6; i++) {
      menu_item_list_add(&items, i + 1, 4, Z2_ITEM_BOTTLE, bottle_contents,
                         sizeof(bottle_contents),
                         &z2_file.items[Z2_SLOT_BOTTLE_1 + i], NULL, -1,
                         &list_sprite, "bottle");
    }

    for (int i = 0; i < 3; i++) {
      menu_item_list_add(&items, 6, i + 1, Z2_ITEM_MOONS_TEAR,
                         trade_quest_contents, sizeof(trade_quest_contents),
                         &z2_file.items[Z2_SLOT_QUEST_1 + i * 6], NULL, -1,
                         &list_sprite, "trade quest");
    }
    ...
    for (int i = 0; i < opt_cnt; i++) {
      ...
      item = menu_item_list_add(&items, 0, i + 1, capacity_options[i].item_tile,
                                options, option_cnt, &capdata->value,
                                capacity_options[i].cap_vals,
                                capacity_options[i].tiles_cnt, &list_sprite,
                                capacity_options[i].tooltip);
      menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                               capacity_event, capdata);
    }
  }
  ...
  {
    masks.selected_item =
        menu_button_add(&masks, 0, 0, "return", menu_return, NULL);
    ...
    for (int i = 0; i < mask_cnt; i++) {
      ...
      data->map_idx = i;
#ifndef LITE
      item = menu_switch_add(&masks, i % 6, i / 6 + 1,
                             get_item_texture(mask_map_table[i].item, 0), NULL,
                             DEFAULT_COLOR, DEFAULT_COLOR, 0, 1, 16, 16, NULL);
#else
  ...
  // Populate quest status menu
  {
    ...
    quest_status.selected_item =
        menu_button_add(&quest_status, 0, 0, "return", menu_return, NULL);
    ...
    menu_item_register_event(item, MENU_EVENT_NUMBER | MENU_EVENT_UPDATE,
                             max_health_event, NULL);

    item = menu_checkbox_add(&quest_status, 0, 2);
    menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                             double_defense_event, NULL);
    ...
    menu_item_register_event(item, MENU_EVENT_LIST, selected_dungeon_onlist,
                             NULL);

    menu_label_add(&quest_status, 0, 6, "small keys");
    item = menu_number_input_add(&quest_status, 15, 6, 16, 2);
    menu_item_register_event(item, MENU_EVENT_NUMBER | MENU_EVENT_UPDATE,
                             dungeon_keys_event, (void *)0);

    menu_label_add(&quest_status, 0, 7, "stray fairies");
    item = menu_number_input_add(&quest_status, 15, 7, 16, 2);
    menu_item_register_event(item, MENU_EVENT_NUMBER | MENU_EVENT_UPDATE,
                             stray_fairies_event, (void *)1);

    gfx_texture *dungeon_items_tex = resource_get(R_Z2_DUNGEON);

    item =
        menu_switch_add(&quest_status, 0, 8, dungeon_items_tex, NULL,
                        DEFAULT_COLOR, DEFAULT_COLOR, 6,
                        dungeon_items_tex->y_tiles / 2 + 6, 16, 16, "boss key");
    menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                             dungeon_item_event, (void *)1);
    menu_item_offset_set(item, 0, 5);

    item =
        menu_switch_add(&quest_status, 1, 8, dungeon_items_tex, NULL,
                        DEFAULT_COLOR, DEFAULT_COLOR, 7,
                        dungeon_items_tex->y_tiles / 2 + 7, 16, 16, "compass");
    menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                             dungeon_item_event, (void *)2);
    menu_item_offset_set(item, 10, 5);

    item = menu_switch_add(&quest_status, 2, 8, dungeon_items_tex, NULL,
                           DEFAULT_COLOR, DEFAULT_COLOR, 8,
                           dungeon_items_tex->y_tiles / 2 + 8, 16, 16, "map");
    menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                             dungeon_item_event, (void *)4);
    menu_item_offset_set(item, 20, 5);

    for (int i = 0;
         i < sizeof(quest_status_table) / sizeof(*quest_status_table); i++) {
      item = menu_switch_add(&quest_status, i, 9,
                             get_item_texture(Z2_ITEM_ODOLWAS_REMAINS + i, 0),
                             NULL, DEFAULT_COLOR, DEFAULT_COLOR, 0, 1, 16, 16,
                             quest_status_table[i].tooltip);
      menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                               quest_status_event,
                               (void *)quest_status_table[i].bitmask);
    ...
    for (int i = 0; i < sizeof(owl_data_table) / sizeof(*owl_data_table); i++) {
      item = menu_switch_add(&quest_status, i % 5, i / 5 + 11, owl_icon_texture,
                             NULL, DEFAULT_COLOR, DEFAULT_COLOR, 0, 1, 16, 8,
                             owl_data_table[i].tooltip);
      menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                               owl_event, (void *)owl_data_table[i].bitmask);
      menu_item_offset_set(item, i % 5 * 6, ((i / 5) * 9) + 21);
    }

    gfx_texture *note_texture = resource_get(R_Z2_NOTE);

    for (int i = 0; i < sizeof(song_data_table) / sizeof(*song_data_table);
         i++) {
      item = menu_switch_add(&quest_status, i % 5 + 6, i / 5 + 11, note_texture,
                             NULL, song_data_table[i].color, 0x808080FF, 0, 0,
                             10, 16, song_data_table[i].tooltip);
      menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
                               quest_status_event,
                               (void *)song_data_table[i].bitmask);
  ...
  // Populate amounts menu
  {
    ...
    amounts.selected_item =
        menu_button_add(&amounts, 0, 0, "return", menu_return, NULL);

    uint8_t items[] = {Z2_ITEM_HEROS_BOW, Z2_ITEM_BOMB, Z2_ITEM_BOMBCHU,
                       Z2_ITEM_STICK,     Z2_ITEM_NUT,  Z2_ITEM_MAGIC_BEAN,
                       Z2_ITEM_POWDER_KEG};

    for (int i = 0; i < sizeof(items) / sizeof(*items); i++) {
      menu_static_gfx_add(&amounts, i % 3 * 3, i / 3 + 1,
                          get_item_texture(items[i], 0), 0, 12, 12);
      item = menu_number_input_add(&amounts, i % 3 * 3 + 1, i / 3 + 1, 10, 3);
      menu_item_register_event(item, MENU_EVENT_NUMBER | MENU_EVENT_UPDATE,
                               menu_number_byte_event, &z2_file.ammo[items[i]]);
    ...
    menu_item_register_event(item, MENU_EVENT_NUMBER | MENU_EVENT_UPDATE,
                             menu_number_byte_event, &z2_file.current_magic);
    menu_item_offset_set(item, 5, 2);

    menu_static_gfx_add(&amounts, 6, 3, dungeon_tex, 1, 12, 12);
    item = menu_number_input_add(&amounts, 7, 3, 16, 4);
    menu_item_register_event(item, MENU_EVENT_NUMBER | MENU_EVENT_UPDATE,
                             menu_number_halfword_event,
                             &z2_file.current_health);
    menu_item_offset_set(item, 5, 2);

    item = menu_static_gfx_add(&amounts, 0, 4, resource_get(R_Z2_RUPEE), 0, 12,
                               12);
    item->color = 0xC8FF64FF;
    item = menu_number_input_add(&amounts, 1, 4, 10, 3);
    menu_item_register_event(item, MENU_EVENT_NUMBER | MENU_EVENT_UPDATE,
                             menu_number_halfword_event, &z2_file.rupees);
    menu_item_offset_set(item, 5, 2);
  }
