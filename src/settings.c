...
#endif
  settings->binds[KZ_CMD_SAVE_MEMFILE] =
      make_bind(2, BUTTON_C_LEFT, BUTTON_D_LEFT);
  settings->binds[KZ_CMD_LOAD_MEMFILE] =
      make_bind(2, BUTTON_C_LEFT, BUTTON_D_RIGHT);
  settings->binds[KZ_CMD_PREV_MEMFILE] = BIND_END;
  settings->binds[KZ_CMD_NEXT_MEMFILE] = BIND_END;
  settings->binds[KZ_CMD_SAVE_POSITION] =
      make_bind(2, BUTTON_C_RIGHT, BUTTON_D_LEFT);
  settings->binds[KZ_CMD_LOAD_POSITION] =
      make_bind(2, BUTTON_C_RIGHT, BUTTON_D_RIGHT);
...
void save_settings_to_flashram(int profile) {
  ...
  memcpy(settings_temp + IO_BLOCK_SIZE + profile * sizeof(settings_info),
         &settings_info, sizeof(settings_info));
...
void load_settings_from_flashram(int profile) {
  ...
  char *settings_temp = malloc(blk_cnt * IO_BLOCK_SIZE);
  z2_dmaflashtoram(settings_temp, SIZE_TO_BLOCK(SETTINGS_ADDR), blk_cnt);
  struct settings *settings_profile =
      (struct settings *)(settings_temp + IO_BLOCK_SIZE +
                          profile * sizeof(*settings_profile));
  ...
  if (settings_profile->header.version == FULL_SETTINGS) {
    if (settings_profile->header.magic_32 == SETTINGS_MAGIC) {
      memcpy((void *)&settings_info, (void *)settings_profile,
             sizeof(*settings_profile));
      kz_log("loaded settings profile %d", profile);
      if ((cur_cheats & (1 << CHEAT_FREEZE_TIME)) &&
          !(settings_profile->data.cheats & (1 << CHEAT_FREEZE_TIME))) {
    ...
    // if settings version is not the same as the current version, delete the
    // profile from the save file, and load default settings in the future might
