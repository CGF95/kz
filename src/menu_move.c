...
static int menu_move_event(event_handler_t *handler, menu_event_t event,
                           void **event_data) {
...
static void menu_move_draw(menu_item_t *item) {
  ...
  gfx_draw_sprite_color(resource_get(R_Z2_ICON), menu_item_x(item),
                        menu_item_y(item), 2, 8, 8, color);
}
...
menu_item_t *menu_move_button_add(menu_t *menu, uint16_t x, uint16_t y,
  ...
  if (item) {
    ...
    menu_item_register_event(item, MENU_EVENT_NAVIGATE | MENU_EVENT_ACTIVATE,
                             menu_move_event, NULL);
  }
  return item;
}