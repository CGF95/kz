...
static int menu_text_input_event(event_handler_t *handler, menu_event_t event,
                                 void **event_data) {
...
menu_item_t *menu_text_input_add(menu_t *menu, uint16_t x_cell, uint16_t y_cell,
  ...
  if (item) {
    ...
    menu_item_register_event(
        item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE | MENU_EVENT_KEYBOARD,
        menu_text_input_event, NULL);
  }
  return item;
}