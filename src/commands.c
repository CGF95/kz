...
void command_timer(void) { kz.timer_running = !kz.timer_running; }
...
void command_fall(void) { z2_link.common.pos_1.y = -32000.0f; }
...
void command_save_memfile(void) {
  ...
  // Save 3 day clock day switching boundaries to prevent day/night advancing
  // when loading outside current day/night
  for (z2_actor_t *actor = z2_game.actor_ctxt.actor_list[0].first; actor;
       actor = actor->next) {
    if (actor->id == 0x15A) {
      z2_timer_t *timer = (z2_timer_t *)actor;
      memcpy(&newmemfile->timer_boundaries, &timer->timer_boundaries,
             sizeof(newmemfile->timer_boundaries));
      break;
    }
  }
  ...
  if (memfile) {
    if (memfile->z2_version == Z2_VERSION) {
      for (z2_actor_t *actor = z2_game.actor_ctxt.actor_list[0].first; actor;
        ...
        if (actor->id == 0x15A) {
          z2_timer_t *timer = (z2_timer_t *)actor;
          memcpy(&timer->timer_boundaries, &memfile->timer_boundaries,
                 sizeof(timer->timer_boundaries));
        }
      }
