 ...
 * For KZ-VC, memory is allocated from each of the frame buffers, and then via
 * sbrk. For KZ-lite, memory is allocated from the 256k at the end of the actor
 * heap that is reserved for kz, and then via sbrk. For KZ-Full, these functions
 * are not used, and newlib's memory managment is used.
 */
...
#define HEAP_ALIGN(s)                                                          \
  (((s) + _Alignof(max_align_t) - 1) & ~(_Alignof(max_align_t) - 1))
    ...
    for (blk = heap_first_free[i]; blk != NULL; blk = blk->next) {
      if (blk->free && blk->size >= size) {
        ...
        if (blk == heap_first_free[i]) {
          for (next_free = blk->next; next_free != NULL;
