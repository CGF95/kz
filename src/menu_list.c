...
static int menu_list_event(event_handler_t *handler, menu_event_t event,
                           void **event_data) {
...
menu_item_t *menu_list_add(menu_t *menu, uint16_t x_cell, uint16_t y_cell,
  ...
  if (item) {
    ...
    menu_item_register_event(
        item, MENU_EVENT_NAVIGATE | MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
        menu_list_event, NULL);
  }
  return item;
}
