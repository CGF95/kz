...
void menu_checkbox_draw(menu_item_t *item) {
  ...
  gfx_draw_sprite_color(checkbox_tex, x, y, 0, checkbox_tex->tile_width,
                        checkbox_tex->tile_height, color);
  if (data) {
    gfx_draw_sprite(checkbox_tex, x, y, 1, checkbox_tex->tile_width,
                    checkbox_tex->tile_height);
  }
}

int menu_checkbox_onactivate(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
...
menu_item_t *menu_checkbox_add(menu_t *menu, uint16_t x_cell, uint16_t y_cell) {
  ...
  if (item) {
    ...
    item->interactive = 1;
    menu_item_register_event(item, MENU_EVENT_ACTIVATE,
                             menu_checkbox_onactivate, (void *)false);
  }
  return item;
}