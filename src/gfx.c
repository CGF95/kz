...
static void rdp_mode_apply(enum rdp_mode mode) {
  ...
  case RDP_MODE_COLOR: {
    ...
    gDPSetPrimColor(gfx_disp_p++, 0, 0, (color >> 24) & 0xFF,
...
void gfx_init() {
  ...
  gfx_disp_p = gfx_disp;
  gfx_disp_d =
      gfx_disp + ((GFX_SIZE + sizeof(*gfx_disp_d) - 1) / (sizeof(*gfx_disp_d)));
...
void gfx_load_tile(gfx_texture *texture, uint16_t tilenum) {
  ...
  if (texture->img_size == G_IM_SIZ_4b) {
    gDPLoadTextureTile_4b(gfx_disp_p++, data, texture->img_fmt,
                          texture->tile_width, texture->tile_height, 0, 0,
                          texture->tile_width - 1, texture->tile_height - 1, 0,
                          G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP,
                          G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
  } else {
    gDPLoadTextureTile(gfx_disp_p++, data, texture->img_fmt, texture->img_size,
                       texture->tile_width, texture->tile_height, 0, 0,
                       texture->tile_width - 1, texture->tile_height - 1, 0,
                       G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP,
                       G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
...
void gfx_texture_desaturate(void *data, size_t len) {
  ...
  MtxF m = guDefMtxF(0.3086f, 0.6094f, 0.0820f, 0.f, 0.3086f, 0.6094f, 0.0820f,
                     0.f, 0.3086f, 0.6094f, 0.0820f, 0.f, 0.f, 0.f, 0.f, 1.f);
...
void gfx_draw_sprite_color(gfx_texture *texture, int x, int y, int tile,
                           int width, int height, uint32_t color) {
  gfx_load_tile(texture, tile);
  rdp_mode_set_apply(RDP_MODE_COLOR, color);
  rdp_mode_replace(RDP_MODE_COMBINE,
  ...
  gSPScisTextureRectangle(
      gfx_disp_p++, qs102(x) & ~3, qs102(y) & ~3,
      qs102(x + texture->tile_width * x_scale_calc + 1) & ~3,
      qs102(y + texture->tile_height * y_scale_calc + 1) & ~3, G_TX_RENDERTILE,
      qu105(0), qu105(0), qu510(1.0f / x_scale_calc),
      qu510(1.0f / y_scale_calc));
  rdp_mode_pop(RDP_MODE_FILTER);
  rdp_mode_pop(RDP_MODE_COMBINE);
  rdp_synced = 0;
}

void gfx_draw_sprite(gfx_texture *texture, int x, int y, int tile, int width,
                     int height) {
...
void gfx_draw_rectangle(int x, int y, int width, int height, uint32_t color) {
  ...
  gSPScisTextureRectangle(gfx_disp_p++, qs102(x), qs102(y), qs102(x + width),
                          qs102(y + height), 0, 0, 0, 0, 0);
  rdp_mode_pop(RDP_MODE_COMBINE);
}

/* text printing functions */

static void gfx_printf_va_color_fast(uint16_t left, uint16_t top,
  ...
  for (int i = 0; i < s_len; i++, x += font->c_width) {
    ...
    gfx_char_t gchar = {tile_char, tmp_color, x, top + 1};
  ...
  for (int i = 0; i < s_len; i++, x += font->c_width) {
    ...
    int tile_idx = c / chars_per_tile;
    int tile_char = c % chars_per_tile;
    gfx_char_t gchar = {tile_char, color, x, top};
    vector_push_back(&gfx_chars[tile_idx], 1, &gchar);
  }
}
...
static void draw_chars(void) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 0; j < tile_vec->size; j++) {
      ...
      first = 0;
      gSPScisTextureRectangle(
          gfx_disp_p++, qs102(gchar->x), qs102(gchar->y),
          qs102(gchar->x + font->c_width), qs102(gchar->y + font->c_height),
          G_TX_RENDERTILE, qu105(gchar->tile % font->cx_tile * font->c_width),
          qu105(gchar->tile / font->cx_tile * font->c_height), qu510(1),
          qu510(1));
      rdp_synced = 0;
    }
...
void gfx_printf_color(uint16_t left, uint16_t top, uint32_t color,
                      const char *format, ...) {
  va_list args;
  va_start(args, format);
...
#endif
    z2_DecodeArchiveFile(z2_file_table[z2_item_icon_archive].prom_start,
                         item_id, tex_buf, size);
...
static gfx_texture *gfx_load_archive(texture_loader *loader) {
  ...
  if (texture) {
    ...
    for (i = 0, j = loader->start_item; i < tiles_cnt; i++, j++) {
      z2_DecodeArchiveFile(z2_file_table[loader->file].prom_start, j, tex_buf,
                           tile_size);
      ...
      if (loader->desaturate) {
        ...
        hmemcpy((char *)texture->data + (tile_size * tiles_cnt) +
                    (tile_size * i),
                tex_buf, tile_size);
      }
    }
...
static gfx_texture *gfx_load_from_rom(texture_loader *loader) {
  ...
  if (texture) {
    ...
    void *tempdata =
        malloc(z2_file_table[file].vrom_end - z2_file_table[file].vrom_start);
    zu_file_idx_load(file, tempdata);

    for (int i = 0; i < loader->num_tiles; i++) {
#ifdef WIIVC
      hmemcpy(texture->data + (i * tile_size),
              (char *)tempdata + loader->offset + (i * tile_size), tile_size);
#else
      memcpy(texture->data + (i * tile_size),
             (char *)tempdata + loader->offset + (i * tile_size), tile_size);
#endif
      if (loader->desaturate) {
        gfx_texture_desaturate(
            (char *)tempdata + loader->offset + (i * tile_size), tile_size);
#ifdef WIIVC
        hmemcpy((char *)texture->data + (tile_size * loader->num_tiles) +
                    (tile_size * i),
                (char *)tempdata + loader->offset + (i * tile_size), tile_size);
#else
        memcpy((char *)texture->data + (tile_size * loader->num_tiles) +
                   (tile_size * i),
               (char *)tempdata + loader->offset + (i * tile_size), tile_size);
#endif
      }
