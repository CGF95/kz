...
__attribute__((section(".data"))) kz_ctxt_t kz = {
    .ready = 0,
};
...
static void kz_main(void) {
  ...
  // Emergency settings reset, konami code
  {
    static uint16_t settings_reset[] = {
        BUTTON_D_UP,   BUTTON_D_UP,    BUTTON_D_DOWN, BUTTON_D_DOWN,
        BUTTON_D_LEFT, BUTTON_D_RIGHT, BUTTON_D_LEFT, BUTTON_D_RIGHT,
        BUTTON_B,      BUTTON_A};
  ...
  /* input display */
  {
    if (settings->input_display) {
      ...
      static const int8_t btns[] = {15, 14, 12, 13, 3,  2, 0,
                                    1,  5,  4,  11, 10, 8, 9};
      ...
      for (int i = 0; i < sizeof(btns) / sizeof(*btns); i++) {
        ...
        gfx_draw_sprite_color(btn_tex, x + 80 + (i * 10), y, btn, 8, 8,
                              button_colors[btn]);
      }
    }
  }
  ...
  {
    for (watch_t *watch = kz.watches.first; watch != NULL;
         watch = list_next(watch)) {
      if (watch->floating) {
  ...
  if (settings->timer) {
    ...
    seconds %= 60;
    minutes %= 60;
    gfx_printf(settings->timer_x, settings->timer_y, "%d:%02d:%02d.%d", hours,
  ...
  /* activate cheats */
  {
    ...
    if (settings->cheats & (1 << CHEAT_TURBO)) {
      ...
      gfx_printf_color(Z2_SCREEN_WIDTH - 60, Z2_SCREEN_HEIGHT - 60, COLOR_GREEN,
                       "t");
    }
    ...
    if (settings->cheats & (1 << CHEAT_RESTRICTION)) {
      ...
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          if (z2_file.form_button_item[i].button_item[j] ==
  ...
  /* handle menu */
  {
    ...
    if (kz.menu_active) {
      if (input_bind_pressed_raw(KZ_CMD_TOGGLE_MENU)) {
        kz.menu_active = 0;
        free_buttons(BUTTON_L | BUTTON_D_DOWN | BUTTON_D_LEFT | BUTTON_D_RIGHT |
                     BUTTON_D_UP);
      ...
      kz.menu_active = 1;
      reserve_buttons(BUTTON_L | BUTTON_D_DOWN | BUTTON_D_LEFT |
                      BUTTON_D_RIGHT | BUTTON_D_UP);
    }
  }
  /* handle command bindings */
  {
    ...
    _Bool is_pause =
        z2_player_ovl_cur == &z2_player_ovl_table[0] && p_ctx->state == 6;
    ...
    if (is_pause && !kz.menu_active) {
      ...
      if (button_time(button_get_index(BUTTON_L)) >= INPUT_REPEAT &&
          !switched) {
        switched = 1;
        ...
        if (item_update) {
          ...
          prev_inst[0] =
              *(uint32_t *)(z2_player_ovl_cur->ram + mask_right_offset);
          prev_inst[1] =
              *(uint32_t *)(z2_player_ovl_cur->ram + mask_left_offset);
          prev_inst[2] =
              *(uint32_t *)(z2_player_ovl_cur->ram + item_right_offset);
          prev_inst[3] =
              *(uint32_t *)(z2_player_ovl_cur->ram + item_left_offset);
          *(uint32_t *)(z2_player_ovl_cur->ram + mask_right_offset) =
              0xA600023E;
        ...
        if (!switched && input_pressed() & BUTTON_L) {
          ...
          case Z2_QUEST_SWORD:
            ...
            if (z2_file.sword == 0) {
              ...
              z2_file.form_button_item[0].b =
                  Z2_ITEM_KOKIRI_SWORD + z2_file.sword - 1;
            }
  ...
  if (kz.pending_frames == 0) {
    gfx_draw_sprite(resource_get(R_Z2_ICON), Z2_SCREEN_WIDTH - 40, 20, 3, 20,
                    20);
  } else if (kz.pending_frames > 0) {
    gfx_draw_sprite(resource_get(R_Z2_ICON), Z2_SCREEN_WIDTH - 40, 20, 4, 20,
                    20);
  }
...
#define MAKESTRING_(S) #S
  {
    ...
    if (logo_time > 0) {
      ...
      gfx_printf_color(10, Z2_SCREEN_HEIGHT - 32 - kfont->c_height, COLOR_GREEN,
                       name);
      gfx_printf_color(Z2_SCREEN_WIDTH - 10 - (kfont->c_width * strlen(url)),
                       Z2_SCREEN_HEIGHT - 32 - kfont->c_height, COLOR_GREEN,
                       url);
      gfx_printf_color(10, Z2_SCREEN_HEIGHT - 20 - kfont->c_height, COLOR_GREEN,
                       version);

      gfx_draw_sprite(resource_get(R_KZ_KZFLEX), Z2_SCREEN_WIDTH - 61,
                      Z2_SCREEN_HEIGHT - 106, 0, 51, 32);
      gfx_draw_sprite(resource_get(R_KZ_KZFLEX), Z2_SCREEN_WIDTH - 61,
                      Z2_SCREEN_HEIGHT - 74, 1, 51, 32);
...
#undef MAKESTRING
  ...
  for (int i = KZ_LOG_MAX - 1; i >= 0; i--) {
    ...
    gfx_printf_color(x, y - (i * 10), GPACK_RGB24A8(0xFFFFFF, alpha), "%s",
                     log_entry->mesg);
  }
...
static int main_menu_return_onactivate(event_handler_t *handler,
...
static int pause_menu_event(event_handler_t *handler, menu_event_t event,
                            void **event_data) {
...
static void init(void) {
  ...
  kz_resource_init();
  gfx_init();
  rdp_mode_set(RDP_MODE_COMBINE,
               G_CC_MODE(G_CC_MODULATEIA_PRIM, G_CC_MODULATEIA_PRIM));

  kz.menu_active = 0;
  menu_init(&kz.main_menu, settings->menu_x, settings->menu_y);
  kz.main_menu.selected_item = menu_button_add(
      &kz.main_menu, 0, 0, "return", main_menu_return_onactivate, NULL);
...
#define Y 4
  ...
  menu_submenu_add(&kz.main_menu, 0, Y + 1, "inventory",
...
#undef Y
  ...
  static menu_sprite_t sprite = {NULL,       0,  0,  COLOR_GREEN,
                                 0x64FF78FF, 22, 22, NULL,
                                 0x64FF78FF, 0,  0,  NULL};

  kz.pause_item_list =
      menu_item_list_add(&kz.pause_menu, 0, 0, 0, NULL, Z2_ITEM_END, &item_val,
                         NULL, Z2_ITEM_END, &sprite, NULL);
  kz.pause_menu.selected_item = kz.pause_item_list;
  kz.pause_item_list->draw_proc = NULL;
  menu_item_register_event(kz.pause_item_list, MENU_EVENT_ACTIVATE,
                           pause_menu_event, NULL);
  ...
  memcpy(restriction_table, &z2_restriction_table,
...
static void game_state_main(void) {
  if (kz.pending_frames != 0) {
    ...
    if (z2_ctxt.gamestate_frames != 1) {
      ...
      gDPSetColorImage(gfx->poly_opa.p++, G_IM_FMT_RGBA, G_IM_SIZ_16b,
                       Z2_SCREEN_WIDTH, 0x0F000000);
      ...
      gDPFillRectangle(gfx->poly_opa.p++, 0, 0, Z2_SCREEN_WIDTH - 1,
                       Z2_SCREEN_HEIGHT - 1);
...
HOOK void draw_room_hook(z2_game_t *game, z2_room_t *room, uint32_t flags) {
  if (kz.hide_room) {
    zu_disp_ptr_t disp_p;
    zu_disp_ptr_save(&disp_p);
    z2_DrawRoom(game, room, flags);
    zu_disp_ptr_load(&disp_p);
  } else {
    z2_DrawRoom(game, room, flags);
  }
