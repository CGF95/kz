...
static enum watch_type watch_type_values[] = {
    WATCH_TYPE_U8,  WATCH_TYPE_S8,   WATCH_TYPE_X8,  WATCH_TYPE_U16,
    WATCH_TYPE_S16, WATCH_TYPE_X16,  WATCH_TYPE_U32, WATCH_TYPE_S32,
    WATCH_TYPE_X32, WATCH_TYPE_FLOAT};

static uint8_t watch_data_sizes[] = {
    1, 1, 1, 2, 2, 2, 4, 4, 4, 4,
};

const int watch_types_cnt =
    sizeof(watch_type_values) / sizeof(*watch_type_values);

static menu_sprite_t delete_sprite = {
    NULL, 0, 0, DEFAULT_COLOR, DEFAULT_COLOR, 8, 8, NULL, 0, 0, 0, NULL};

static menu_sprite_t anchor_sprite = {
    NULL, 2, 2, DEFAULT_COLOR, DEFAULT_COLOR, 8, 8, NULL, 0, 0, 0, NULL};
...
static int watch_address_onnumber(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
...
static int menu_watch_delete_onactivate(event_handler_t *handler,
  ...
  while (next) {
    for (int i = 0; i < sizeof(next->row_items) / sizeof(*next->row_items);
         i++) {
...
static int menu_watch_anchor_onactivate(event_handler_t *handler,
                                        menu_event_t event, void **event_data) {
  struct watch_row *row = handler->callback_data;
...
static int watch_type_onlist(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
  struct watch_row *row = handler->callback_data;
...
static void watch_add(watch_t *watch, _Bool setpos) {
  ...
  row->delete_button = menu_gfx_button_add(&watches_menu, x, y, &delete_sprite,
                                           menu_watch_delete_onactivate, row);

  menu_sprite_t *anchor = malloc(sizeof(*anchor));
  memcpy(anchor, &anchor_sprite, sizeof(*anchor));
  anchor->on_tile = watch->floating ? 1 : 7;
  row->anchor_button = menu_gfx_button_add(&watches_menu, x + 1, y, anchor,
                                           menu_watch_anchor_onactivate, row);

  row->move_button =
      menu_move_button_add(&watches_menu, x + 2, y, &watch->x, &watch->y);
  row->move_button->enabled = watch->floating;

  row->addr_input = menu_number_input_add(&watches_menu, x + 4, y, 16, 8);
  menu_item_register_event(row->addr_input, MENU_EVENT_NUMBER,
                           watch_address_onnumber, row);
  menu_number_set(row->addr_input, (uint32_t)watch->address);

  row->type_list = menu_list_add(&watches_menu, x + 14, y, watch_type_names,
                                 watch_types_cnt);
  menu_item_register_event(row->type_list, MENU_EVENT_LIST, watch_type_onlist,
                           row);
  ...
  row->watch = watch;
  row->label_item = menu_text_input_add(&watches_menu, x + 4, y + 1, "label",
                                        &watch->label, 21);
  add_button->y_cell += 2;
}

static int watches_button_add_onactivate(event_handler_t *handler,
                                         menu_event_t event,
                                         void **event_data) {
...
menu_t *create_watches_menu(void) {
  ...
  watches_menu.selected_item =
      menu_button_add(&watches_menu, 0, 0, "return", menu_return, NULL);
  add_button = menu_button_add(&watches_menu, 0, 1, "+",
                               watches_button_add_onactivate, NULL);
  init_watch_rows();
  return &watches_menu;
}
