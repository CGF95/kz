...
void watch_printf(watch_t *watch, uint32_t color) {
  ...
  case WATCH_TYPE_FLOAT: {
    ...
    /* moderately annoying workaround for double rounding issues in _etoa and
     * _ftoa, this doesn't solve the rounding issues, but rather fixes a common
     * case of 0.0 */
  ...
  if (watch->label && watch->floating) {
    gfx_printf_color(watch->x + ((strlen(buf) + 1) * 8), watch->y, color, "%s",
                     watch->label);
  }
}
