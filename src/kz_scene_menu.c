...
static int collision_gen_event(event_handler_t *handler, menu_event_t event,
                               void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
    ...
    menu_checkbox_set(handler->subscriber,
...
static int collision_switch_event(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
...
static int hitbox_gen_event(event_handler_t *handler, menu_event_t event,
                            void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
...
static int hitbox_opaque_event(event_handler_t *handler, menu_event_t event,
                               void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
...
static int unload_room_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
...
static int load_room_onactivate(event_handler_t *handler, menu_event_t event,
                                void **event_data) {
...
static int hide_actors_onactivate(event_handler_t *handler, menu_event_t event,
                                  void **event_data) {
  kz.hide_actors = !kz.hide_actors;
  return 1;
}

static int hide_room_onactivate(event_handler_t *handler, menu_event_t event,
                                void **event_data) {
  kz.hide_room = !kz.hide_room;
  return 1;
}

static int free_cam_event(event_handler_t *handler, menu_event_t event,
                          void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
...
static int lock_cam_event(event_handler_t *handler, menu_event_t event,
                          void **event_data) {
  if (event == MENU_EVENT_ACTIVATE) {
...
static int set_cam_pos_event(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
  sph_coord_t sph;
  z2_xyzf_t at_dir;
...
menu_t *create_scene_menu(void) {
  ...
  scene.selected_item =
      menu_button_add(&scene, 0, 0, "return", menu_return, NULL);
  ...
  // Generate Camera Menu
  {
    ...
    camera.selected_item =
        menu_button_add(&camera, 0, 0, "return", menu_return, NULL);
    ...
    menu_button_add(&camera, 0, 3, "set cam pos to link pos", set_cam_pos_event,
                    NULL);
    /*
    item = menu_checkbox_add(&camera, 0, 3);
    menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_UPDATE,
    cam_draw_event, NULL); menu_label_add(&camera, 2, 3, "draw camera");
    */
  }
  ...
  menu_item_register_event(item, MENU_EVENT_NUMBER, menu_number_word_event,
                           &room_idx);
