...
uint32_t button_colors[16] = {
    0xFFF000FF, 0xFFF000FF, 0xFFF000FF, 0xFFF000FF, 0xFFFFFFFF, 0xFFFFFFFF,
    0,          0,          0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
    0xFF823CFF, 0xFFFFFFFF, 0x64FF78FF, 0x64C8FFFF,
};
...
void bind_override(int index) { bind_overrides[index] = 1; }
...
void input_update(void) {
  ...
  for (int i = 0; i < KZ_CMD_MAX; i++) {
    ...
    int j = 0;
    uint16_t c = 0;
    if (((pad_reserved & bind_pad[i]) && !bind_overrides[i]) ||
        !input_enabled) {
      *cs = 0;
    } else {
      int css = *cs;
...
_Bool input_bind_held(int index) { return bind_time[index] > 0; }

_Bool input_bind_pressed(int index) { return bind_pressed[index]; }

_Bool input_bind_pressed_raw(int index) { return bind_pressed_raw[index]; }

void reserve_buttons(uint16_t button_bitmask) {
  pad_reserved |= button_bitmask;
}

void free_buttons(uint16_t button_bitmask) { pad_reserved ^= button_bitmask; }
...
uint16_t input_pressed_raw(void) { return pad_cur; }

uint16_t pad_pressed_raw(void) { return pressed_raw; }

int8_t input_x(void) { return stick_x; }

int8_t input_y(void) { return stick_y; }

int button_time(int btn) { return __button_time[btn]; }

void input_enable(void) { input_enabled = 1; }

void input_disable(void) { input_enabled = 0; }
