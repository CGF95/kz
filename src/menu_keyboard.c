...
static const char *keyboard_rows[] = {"`1234567890-=\x08", "qwertyuiop[]\\",
                                      "asdfghjkl;'", "\x01zxcvbnm,./\x01"};

static const char *keyboard_shift_rows[] = {
    "~!@#$%^&*()_+\x08", "QWERTYUIOP{}|", "ASDFGHJKL;\"", "\x01ZXCVBNM<>?\x01"};
...
static inline void shift() { shifted = !shifted; }
...
static int keyboard_event(event_handler_t *handler, menu_event_t event,
                          void **event_data) {
...
static menu_item_t *menu_keyboard_input_add(menu_t *menu, int x, int y) {
  ...
  if (item) {
    ...
    item->interactive = 1;
    menu_item_register_event(item, MENU_EVENT_UPDATE | MENU_EVENT_NAVIGATE,
                             keyboard_event, NULL);
  }
  return item;
}
...
static void key_draw(menu_item_t *item) {
  ...
  if (item->owner->selected_item == item) {
    gfx_draw_rectangle(menu_item_x(item) - 4, menu_item_y(item) - 4, 16, 16,
                       SELECTED_COLOR);
  }
  char c = source[data->row][data->col];
  if (c == 0x01) {
    gfx_draw_sprite(resource_get(R_Z2_ICON), menu_item_x(item),
                    menu_item_y(item), 5, 8, 8);
  } else if (c == 0x08) {
    gfx_draw_sprite(resource_get(R_Z2_ICON), menu_item_x(item),
                    menu_item_y(item), 6, 8, 8);
  } else {
    gfx_printf(menu_item_x(item), menu_item_y(item), "%c",
               source[data->row][data->col]);
  }
}

static int key_event(event_handler_t *handler, menu_event_t event,
                     void **event_data) {
...
static menu_item_t *menu_key_add(menu_t *menu, int x_cell, int y_cell, int row,
                                 int col) {
  ...
  if (item) {
    ...
    item->interactive = 1;
    menu_item_register_event(item, MENU_EVENT_ACTIVATE | MENU_EVENT_NAVIGATE,
                             key_event, NULL);
  }
  return item;
}
...
static void legend_draw(menu_item_t *item) {
  ...
  gfx_draw_sprite_color(resource_get(R_KZ_BUTTONS), x + 10, y, 9, 8, 8,
                        0xFFFFFFFF);
  gfx_printf(x + 20, y, "backspace");

  gfx_draw_sprite_color(resource_get(R_KZ_BUTTONS), x, y + 10, 4, 8, 8,
                        0xFFFFFFFF);
  gfx_draw_sprite_color(resource_get(R_KZ_BUTTONS), x + 10, y + 10, 8, 8, 8,
                        0xFFFFFFFF);
  gfx_printf(x + 20, y + 10, "space");

  gfx_draw_sprite_color(resource_get(R_KZ_BUTTONS), x, y + 20, 4, 8, 8,
                        0xFFFFFFFF);
  gfx_draw_sprite_color(resource_get(R_KZ_BUTTONS), x + 10, y + 20, 11, 8, 8,
                        0xFFFFFFFF);
  gfx_printf(x + 20, y + 20, "caps lock");

  gfx_draw_sprite_color(resource_get(R_KZ_BUTTONS), x, y + 30, 4, 8, 8,
                        0xFFFFFFFF);
  gfx_draw_sprite_color(resource_get(R_KZ_BUTTONS), x + 10, y + 30, 10, 8, 8,
                        0xFFFFFFFF);
  gfx_printf(x + 20, y + 30, "accept");
}

static int accept_onactivate(event_handler_t *handler, menu_event_t event,
                             void **event_data) {
  return accept(event_data);
}

static int keyboard_return(event_handler_t *handler, menu_event_t event,
                           void **event_data) {
  if ((int)*event_data == 1) {
    return 0;
  }
...
static void init_keyboard(void) {
  ...
  menu_register_event(&menu_keyboard, MENU_EVENT_RETURN, keyboard_return,
                      (void *)0);
  menu_keyboard.selected_item =
      menu_button_add(&menu_keyboard, 0, 0, "return", menu_return, (void *)1);
  ...
  menu_item_t *legend =
      menu_label_add(&menu_keyboard, 0,
                     sizeof(keyboard_rows) / sizeof(*keyboard_rows) + 4, NULL);
