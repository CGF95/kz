 ...
 * ((v1.x-v0.x),(v1.y-v0.y),(v1.z-v0.z)) cross
 * ((v2.x-v0.x),(v2.y-v0.y),(v2.z-v0.z)) cross(a,b) = ((aybz - azby),(azbx -
 * axbz),(axby - aybx)) norm = (
 ...
 *        )
 */
static void tri_norm(z2_xyzf_t *v0, z2_xyzf_t *v1, z2_xyzf_t *v2,
                     z2_xyzf_t *norm) {
  norm->x =
      ((v1->y - v0->y) * (v1->z - v0->z)) - ((v1->z - v0->z) * (v2->y - v0->y));
  norm->y =
      ((v1->z - v0->z) * (v2->x - v0->x)) - ((v1->x - v0->x) * (v2->z - v0->z));
  norm->z =
      ((v1->x - v0->x) * (v2->y - v0->y)) - ((v1->y - v1->y) * (v2->x - v0->x));

  float norm_dist =
      sqrtf(norm->x * norm->x + norm->y * norm->y + norm->z * norm->z);
...
void draw_uv_sphere(Gfx **hit_view_p, Gfx **hit_view_d, int16_t radius,
  ...
  if (sphere_gfx == NULL) {
    ...
    for (int i = 0; i < idx_cnt; i += 6) {
      ...
      // Load vertices if the first or last isn't loaded.  This probably could
      ...
      gSP2Triangles(gfx_p++, indices[i + 2] - vtxstart,
                    indices[i + 1] - vtxstart, indices[i] - vtxstart, 0,
                    indices[i + 5] - vtxstart, indices[i + 4] - vtxstart,
                    indices[i + 3] - vtxstart, 0);
    }
  ...
  dlist = HB_SEG(dlist);
#else
  gSPMatrix((*hit_view_p)++, gDisplayListData(hit_view_d, m),
            G_MTX_LOAD | G_MTX_MODELVIEW | G_MTX_PUSH);
#endif
  gSPDisplayList((*hit_view_p)++, dlist);
  gSPPopMatrix((*hit_view_p)++, G_MTX_MODELVIEW);
}

static void draw_cylinder(Gfx **hit_view_p, Gfx **hit_view_d, int16_t radius,
                          int16_t height, int16_t x, int16_t y, int16_t z) {
  ...
  if (cylinder_p == NULL) {
    ...
    for (int i = 0; i < 12; i++) {
      ...
      cylinder_vtx[2 + i * 2] =
          gdSPDefVtxN(vtx_x, 0, vtx_z, 0, 0, norm_x, 0, norm_z, 0xFF);
      cylinder_vtx[3 + i * 2] =
          gdSPDefVtxN(vtx_x, 128, vtx_z, 0, 0, norm_x, 0, norm_z, 0xFF);
    }
    ...
    for (int i = 0; i < 12; i++) {
      int pos = (i == 0 ? 12 : i) - 1;
      int v[4] = {2 + pos * 2 + 0, 2 + i * 2 + 0, 2 + i * 2 + 1,
                  2 + pos * 2 + 1};
...
static void draw_triangle(Gfx **hit_view_p, Gfx **hit_view_d, z2_xyzf_t *v0,
                          z2_xyzf_t *v1, z2_xyzf_t *v2) {
  z2_xyzf_t norm;
...
static void draw_quad(Gfx **hit_view_p, Gfx **hit_view_d, z2_xyzf_t *v0,
                      z2_xyzf_t *v1, z2_xyzf_t *v2, z2_xyzf_t *v3) {
  z2_xyzf_t norm;
...
static void do_hitbox_view(Gfx **hit_view_p, Gfx **hit_view_d, int hitbox_cnt,
                           z2_hitbox_t **hitbox_list, uint32_t color) {

  gDPSetPrimColor((*hit_view_p)++, 0, 0, (color >> 16) & 0xFF,
                  (color >> 8) & 0xFF, (color >> 0) & 0xFF, 0xFF);
  ...
  for (int i = 0; i < hitbox_cnt; i++) {
    ...
    case Z2_HITBOX_SPHERE_LIST: {
      ...
      for (int j = 0; j < sphere_list->entry_cnt; j++) {
        ...
        draw_uv_sphere(hit_view_p, hit_view_d, radius, entry->pos.x,
                       entry->pos.y, entry->pos.z);
      }
    } break;
    case Z2_HITBOX_CYLINDER: {
      ...
      draw_cylinder(hit_view_p, hit_view_d, radius, cylinder->height,
                    cylinder->pos.x, cylinder->pos.y + cylinder->y_offset,
                    cylinder->pos.z);
    } break;
    case Z2_HITBOX_TRI_LIST: {
      ...
      for (int j = 0; j < tri_list->entry_cnt; j++) {
        ...
        draw_triangle(hit_view_p, hit_view_d, &entry->vertices[0],
                      &entry->vertices[2], &entry->vertices[1]);
      }
    } break;
    case Z2_HITBOX_QUAD: {
      z2_hitbox_quad_t *quad = (z2_hitbox_quad_t *)hitbox;
      draw_quad(hit_view_p, hit_view_d, &quad->vertices[0], &quad->vertices[2],
                &quad->vertices[3], &quad->vertices[1]);
    } break;
    case Z2_HITBOX_SPHERE: {
      z2_hitbox_sphere_t *sphere = (z2_hitbox_sphere_t *)hitbox;
      draw_uv_sphere(hit_view_p, hit_view_d, sphere->radius, sphere->pos.x,
...
void kz_hitbox_view() {
  ...
  _Bool show = kz.hitbox_view_status == COL_VIEW_ACTIVE && zu_is_ingame() &&
  ...
  if (show) {
    ...
    do_hitbox_view(&hit_view_p, &hit_view_d, z2_game.hitbox_ctxt.other_cnt,
                   z2_game.hitbox_ctxt.other, 0x00FFFFFF);
    do_hitbox_view(&hit_view_p, &hit_view_d, z2_game.hitbox_ctxt.ac_cnt,
                   z2_game.hitbox_ctxt.ac, 0x000000FF);
    do_hitbox_view(&hit_view_p, &hit_view_d, z2_game.hitbox_ctxt.attack_cnt,
                   z2_game.hitbox_ctxt.attack, 0x00FF0000);
...
static void do_poly_list(poly_writer_t *writer, line_writer_t *line_writer,
                         struct vector *line_set, z2_xyz_t *vtx,
                         z2_col_poly_t *polys, z2_col_type_t *types,
                         int poly_cnt, _Bool redux) {
  for (int i = 0; i < poly_cnt; i++) {
    ...
    if (!skip) {
      Vtx v[3] = {
          gdSPDefVtxN(va->x, va->y, va->z, 0, 0, poly->norm.x / 0x100,
                      poly->norm.y / 0x100, poly->norm.z / 0x100, 0xFF),
          gdSPDefVtxN(vb->x, vb->y, vb->z, 0, 0, poly->norm.x / 0x100,
                      poly->norm.y / 0x100, poly->norm.z / 0x100, 0xFF),
          gdSPDefVtxN(vc->x, vc->y, vc->z, 0, 0, poly->norm.x / 0x100,
                      poly->norm.y / 0x100, poly->norm.z / 0x100, 0xFF),
      };
      ...
      if (!skip) {
        ...
        if (line_set != NULL) {
          for (int i = 0; i < line_set->size; i++) {
            ...
            if ((l->a == lab.a && l->b == lab.b) ||
                (l->a == lab.b && l->b == lab.a)) {
              ab = 0;
            }

            if ((l->a == lbc.a && l->b == lbc.b) ||
                (l->a == lbc.b && l->b == lbc.a)) {
              bc = 0;
            }

            if ((l->a == lac.a && l->b == lac.b) ||
                (l->a == lac.b && l->b == lac.a)) {
              ca = 0;
            }
          }
...
static void do_dynamic_list(poly_writer_t *writer, line_writer_t *line_writer,
                            z2_col_hdr_t *hdr, uint16_t idx) {
  z2_col_ctxt_t *col_ctx = &z2_game.col_ctxt;
  while (idx != 0xFFFF) {
    z2_col_list_t *list = &col_ctx->dyn.list[idx];
    do_poly_list(writer, line_writer, NULL, col_ctx->dyn_vtx,
                 &col_ctx->dyn_poly[list->poly_idx], hdr->type, 1, 0);
    idx = list->list_next;
  }
}
...
static size_t calc_poly_cnt(void) {
  ...
  for (int i = 0; i < col_hdr->n_poly; i++) {
    ...
    if (type->flags_2.hookshot || type->flags_1.interaction > 1 ||
        type->flags_1.special == 0xC || type->flags_1.exit != 0 ||
        type->flags_1.special == 5 || type->flags_1.behavior != 0 ||
...
void kz_col_view() {
  ...
  bool init = kz.collision_view_status == COL_VIEW_START ||
  ...
  if (enabled && init) {
    ...
    do_poly_list(&poly_writer, line_writer_p, &line_set, col_hdr->vtx,
                 col_hdr->poly, col_hdr->type, col_hdr->n_poly,
                 settings->col_view_red);
