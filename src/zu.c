...
void zu_file_load(uint32_t vrom_addr, void *ram, size_t size) {
  ...
  z2_loadfile_t file = {vrom_addr, ram, size, NULL, 0, 0, &queue, 0};
...
void zu_file_idx_load(int idx, void *ram) {
  zu_file_load(z2_file_table[idx].vrom_start, ram,
               z2_file_table[idx].vrom_end - z2_file_table[idx].vrom_start);
}
...
void zu_gfx_reloc(int src_disp_idx, int src_cimg_idx) {
  ...
  z2_disp_buf_t *new_disp[] = {
      &gfx->work,          &gfx->poly_opa, &gfx->poly_xlu, &gfx->overlay,
      &gfx->unk_0x1B8_buf, &primary,       &segment_setup,
  };
...
void zu_seq_vol(int player, float vol) {
  z2_AfxCmdFloat(0x41000000 | (player << 16), vol);
}

void zu_seq_chan_vol(int player, int chan, float vol) {
  z2_AfxCmdFloat(0x01000000 | (player << 16) | (chan << 8), vol);
}

void zu_seq_chan_mute(int player, int chan, _Bool mute) {
  z2_AfxCmdWord(0x08000000 | (player << 16) | (chan << 8), mute << 24);
}

void zu_seq_stop(int player) {
  z2_AfxCmdWord(0x83000000 | (player << 16), 0x00000000);
}
