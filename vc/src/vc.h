...
bool kz_treeSearchNode(func_tree_node_t *node, int n64_addr,
                       func_tree_node_t **found_node);
bool kz_cpuTreeTake(func_tree_node_t **out_node, int *out_pos, size_t size);
bool kz_treeBalance(recomp_ctxt_t *ctx);

/* Homeboy's entry point. Must match the ADDRESS the homeboy binary is
 * linked at (see the homeboy build invocation in the root makefile) and
 * the hb_addr the wad injects it at in build/lua/rom_info.lua. */
...
vc_extern bool cpuTreeTake(func_tree_node_t **out_node, int *out_pos,
                           size_t size);
vc_extern bool treeAdjustRoot(n64_cpu_t *cpu, int n64_start, int n64_end);
vc_extern bool treeInsertNode(func_tree_node_t **start, int n64_start,
                              int n64_end, func_tree_node_t **new_node);
vc_extern bool ramSetSize(void **dst, int size);
vc_extern bool xlObjectMake(void **obj, void *parent, class_type_t *class);
vc_extern bool cpuMapObject(n64_cpu_t *cpu, void *dev, uint32_t addr_start,
                            uint32_t addr_end, uint32_t arg4);
vc_extern bool cpuSetDevicePut(n64_cpu_t *cpu, cpu_dev_t *dev, void *sb,
                               void *sh, void *sw, void *sd);
vc_extern bool cpuSetDeviceGet(n64_cpu_t *cpu, cpu_dev_t *dev, void *lb,
                               void *lh, void *lw, void *ld);
vc_extern bool treeCallerCheck(n64_cpu_t *cpu, func_tree_node_t *node,
                               uint32_t arg2, uint32_t n64_start,
                               uint32_t n64_end);
