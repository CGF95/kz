...
static class_type_t vc_dev_class = {"VC-DEV", sizeof(vc_dev_t), 0,
                                    vc_dev_event};

static bool force_clean(void) {
  func_tree_node_t *node = NULL;
  if (!kz_treeSearchNode(gSystem->cpu->recomp_ctx->code_tree_root,
                         vc_dev->func_to_clean, &node) &&
      !kz_treeSearchNode(gSystem->cpu->recomp_ctx->other_tree_root,
                         vc_dev->func_to_clean, &node) &&
...
static void recomp_stats(void) {
  ...
  for (int i = 0; i < sizeof(gSystem->cpu->sm_blk_status) /
                          sizeof(*gSystem->cpu->sm_blk_status);
       i++) {
  ...
  for (int i = 0; i < sizeof(gSystem->cpu->lg_blk_status) /
                          sizeof(*gSystem->cpu->lg_blk_status);
       i++) {
...
static bool sb(void *callback, uint32_t addr, uint8_t *src) { return false; }

static bool sh(void *callback, uint32_t addr, uint16_t *src) { return false; }
...
static bool sd(void *callback, uint32_t addr, uint64_t *src) { return false; }

static bool lb(void *callback, uint32_t addr, uint8_t *dst) { return false; }

static bool lh(void *callback, uint32_t addr, uint16_t *dst) { return false; }

static bool lw(void *callback, uint32_t addr, uint32_t *dst) { return false; }

static bool ld(void *callback, uint32_t addr, uint64_t *dst) { return false; }
